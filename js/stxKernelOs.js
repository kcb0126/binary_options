...
labelDecimalPlaces=8;//Jacked my Wu.R
...
else{txt=this.formatYAxisPrice(quote.Close,panel,labelDecimalPlaces);}txt=""+quote.Close;//jacked by Wu.R
...
return obj;}
;STXChart.prototype.doCleanupGaps=function(quotes){if(!this.cleanupGaps)return quotes;if(v4W.H9y(interval,(J0V+I5V)))return quotes;if(quotes&&!quotes.length)return quotes;var interval=this.layout.interval;if((v4W.G9y(interval,"month")||v4W.o9y(interval,"week"))&&!this.dontRoll){interval="minute";}
...
for(j in stx.plugins){var plugin=stx.plugins[j];if(!plugin.transformOutputs)continue;for(field in plugin.transformOutputs){if(quote[field]||v4W.j0y(quote[field],0))quote.transform[field]=v4W.q0y(Math.round(((quote[field]-STX.Comparison.baseline)/STX.Comparison.baseline*100)*1000000),1000000);}
}
