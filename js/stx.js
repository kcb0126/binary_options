...
STX.Market.Iterator._Clock.prototype.find_prev=null;
		...
		*/
		...
		STX.DrawingToolbar.setDrawingType=function(vectorType, div){
			div = document.getElementById("toolbarDraw");
			var self=STX.DrawingToolbar.findInstance(div);
			console.log(self);
			self.setVectorType(vectorType);
		};
		...
		STX.QuoteFeed.Demo.prototype.generateDaily=function(params, cb){
			...
			$.ajax({
				...
				success: function(resp) {
					var respObj = resp
//					var quotes = setQuotes(respObj);
                    var quotes = respObj;
					var newQuotes = [];
					for(var i = 0; i < quotes.length; i++) {
						newQuotes[i] = {};
						newQuotes[i].Date = quotes[i].TimeStamp;
						newQuotes[i].Date = newQuotes[i].Date.split('.')[0];
						newQuotes[i].Open = quotes[i].open;
						newQuotes[i].High = quotes[i].high;
						newQuotes[i].Low = quotes[i].low;
						newQuotes[i].Close = quotes[i].close;
					}
